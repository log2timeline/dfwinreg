...
then
	...
	for FILE in `find setup.py config dfwinreg tests -name \*.py`;
	do
		echo "Checking: ${FILE}";
...
then
	...
	docker exec "${CONTAINER_NAME}" sh -c "git clone https://github.com/log2timeline/dfwinreg.git";

	if test ${TRAVIS_PYTHON_VERSION} = "2.7";
	then
		docker exec "${CONTAINER_NAME}" sh -c "cd dfwinreg && python2 run_tests.py";
	else
		docker exec "${CONTAINER_NAME}" sh -c "cd dfwinreg && python3 run_tests.py";
	fi
