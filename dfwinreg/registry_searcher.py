...
class FindSpec:
    ...
    def __init__(self, key_path=None, key_path_glob=None, key_path_regex=None):
        ...
        key_path_arguments = [
            argument
            for argument in (key_path, key_path_glob, key_path_regex)
            if argument
        ]
    ...
    def _CompareWithKeyPathSegment(self, key_path_segment, segment_index):
        ...
        """
        if (
            self._key_path_segments is None
            or segment_index < 0
            or segment_index > self._number_of_key_path_segments
        ):
            return False
        ...
        if self._is_regex:
            result = bool(
                segment_name.match(key_path_segment)
            )  # pylint: disable=no-member
            return result
    ...
    def AtLastKeyPathSegment(self, segment_index):
        ...
        """
        return bool(
            self._key_path_segments is not None
            and (segment_index + 1) >= self._number_of_key_path_segments
        )
